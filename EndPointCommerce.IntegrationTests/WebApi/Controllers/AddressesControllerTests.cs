...
public class AddressesControllerTests : IntegrationTestFixture
{
    ...
    public async Task GetAddresses_ReturnsAddresses_AssociatedWithLoggedInUser_ExcludingDeletedOnes()
    {
        await WithTransaction(async () => {
            ...
            var address3 = CreateNewAddress(_dbContext.States.Skip(10).First(), customer, true);
    ...
    public async Task PostAddress_CreatesNewAddress_AssociatedWithLoggedInUser()
    {
        await WithTransaction(async () =>
        {
            ...
            Assert.Equal(_dbContext.States.First().Id, address.StateId);
            Assert.Equal("12345", address.ZipCode);
            Assert.Equal(_dbContext.Customers.First().Id, address.CustomerId);
        });
    }
    ...
    public async Task PutAddress_UpdatesAddress()
    {
        await WithTransaction(async () =>
        {
            ...
            var updatedAddress = _dbContext.Addresses.Find(address.Id);
            ...
            Assert.Equal(_dbContext.States.Skip(10).First().Id, updatedAddress.StateId);
    ...
    public async Task PutAddress_ReturnsNotFound_WhenAddressDoesNotBelongToLoggedInUser()
    {
        await WithTransaction(async () =>
        {
            ...
            _dbContext.Customers.Add(anotherCustomer);
            _dbContext.SaveChanges();
    ...
    public async Task DeleteAddress_DeletesAddress()
    {
        await WithTransaction(async () =>
        {
            ...
            var deletedAddress = _dbContext.Addresses.Find(address.Id);
