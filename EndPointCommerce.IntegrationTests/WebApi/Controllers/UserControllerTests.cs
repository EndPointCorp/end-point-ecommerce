...
public class UserControllerTests : IntegrationTestFixture
{
    ...
    public async Task GetUser_ReturnsLoggedInUserData_WhenUserIsLoggedIn()
    {
        await WithTransaction(async () => {
            ...
            Assert.Equal(_dbContext.Customers.First().Id, user.Customer!.Id);
            Assert.Equal(_dbContext.Customers.First().Name, user.Customer!.Name);
            Assert.Equal(_dbContext.Customers.First().LastName, user.Customer!.LastName);
        });
    }
    ...
    public async Task GetUser_ReturnsNotFound_WhenUserIsNotACustomer()
    {
        await WithTransaction(async () => {
            ...
            var user = _dbContext.Users.First(u => u.Email == "test@email.com");
