...
- `CATEGORY_IMAGES_URL_PATH`, `PRODUCT_IMAGES_URL_PATH` determines the URLs of product and category images. Used by the Web API to link to them. In most cases these would both be set to the category and product image URLs from the Web API container.
...
- In `EndPointCommerce.WebApi/appsettings.json` (or `EndPointCommerce.WebApi/appsettings.Local.json`), set:
  ...
  - The `TaxJar*` settings to that of your TaxJar account.
  - The `AuthNet*` settings to that of your Authorize.NET account.
  - `CategoryImagesUrlPath` to `WEB_API_URL/category-images`.
  - `ProductImagesUrlPath` to `WEB_API_URL/product-images`.
  - `WebStorePasswordResetUrl` to `WEB_STORE_URL/reset-password`. This is the page in your frotnend where you offer password reset functionality to your users.
