...
3. A sample web store frontend that demonstrates API interactions.
...
- `BUILD_ENV` determines the .NET build mode.
- `CATEGORY_IMAGES_URL_PATH`, `PRODUCT_IMAGES_URL_PATH` determines the URLs of product and category images. Used by the Web API to link to them.
...
- In `EndPointCommerce.WebApi/appsettings.json`, set:
  ...
  - The `TaxJar*` settings to that of your TaxJar account.
  - The `AuthNet*` settings to that of your Authorize.NET account.
  - `WebsiteUrl` to the URL where your frontend is running. For a local development environment, where you want to use the Web Store project, this should be `https://localhost:7166`. This is the default URL that the Web Store starts up at with `dotnet run`.
  - `WebsiteLogoUrl` to `WEB_STORE_URL/logo.png`.
  - `WebsitePasswordResetUrl` to `WEB_STORE_URL/reset-password`.
  - `WebsiteShippingInfoUrl` to `WEB_STORE_URL/shipping-information`.
